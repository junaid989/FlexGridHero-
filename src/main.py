...
from editor import css_challenge
...
def cyber_intro(ui):
    ...
    for s in scan:
        ui.clear()
        ui.header('FlexGrid Hero+ — Cyber Neon')
        print()
        print('Initializing: '+s)
        time.sleep(0.18)
    ui.clear()
    ui.header('FlexGrid Hero+ — Cyber Neon')
    ui.type_writer('Welcome to FlexGrid Hero+ — Learn Flexbox, Nested Flexbox & Grid', speed=0.01)
...
def main():
    ...
    while level <= lm.max_level():
        ...
        while True:
            ...
            if ans.strip().lower() == 'hint':
                ui.print_info(lm.hint(level))
                continue
            if ans.strip().lower() == 'skip':
                ui.print_warn('Skipping level (no badge)')
                level += 1
                break
            if lm.check_answer(level, ans):
                ...
                if badge and badge not in badges:
                    ui.print_badge(badge)
                    badges.add(badge)
                level += 1
                prog['level'] = level
                prog['badges'] = list(badges)
                save_progress(prog)
                break
            else:
    ...
    ui.type_writer('You finished FlexGrid Hero+! Well done.')
    if badges:
        ui.print_info('Badges: ' + ', '.join(sorted(badges)))
    ui.print_info('Progress saved to data/progress.json')
